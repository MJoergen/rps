#ifndef _PLAYER_ANTI_MIKE_H_
#define _PLAYER_ANTI_MIKE_H_
...
class PlayerAntiMike : public Player
{
public:
   PlayerAntiMike(const std::string& name) :
      Player(name),
      m_predict(27*9)
   {}
   ...
   void fillPredict(const std::vector<Move>& moveHistory)
   {
      ...
      for (size_t i=2; i<moveHistory.size(); ++i)
      {
         int ix = moveHistory[i-2].m_you*9*9 + moveHistory[i-2].m_me*3*9 +
            moveHistory[i-1].m_you*9 + moveHistory[i-1].m_me*3 + moveHistory[i].m_me;
         m_predict[ix] += 1;
      }
...
}; // end of PlayerAntiMike

#endif // _PLAYER_ANTI_MIKE_H_
